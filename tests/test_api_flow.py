...
def test_graph_page_renders(logged_in):
    response = logged_in.get(f"/playlists/{PLAYLIST_ID}/graph")
    assert response.status_code == 200
    # The elements graphPage.js reaches for by id. Renaming one in the template
    # without the other is a blank page, not an error.
    assert b'id="graph-canvas"' in response.data
    assert b'id="detail-panel"' in response.data
...
def test_unavailable_mode_is_rejected(logged_in, monkeypatch):
    ...
    The only shipped mode is available, so this stands it down to exercise the
    path. It stays covered because the next mode to be added lands here first,
    and a 400 would tell the front end the mode does not exist rather than
    that it is not ready.
    """
    monkeypatch.setattr(ArtistNetworkBuilder, "available", False)

    response = logged_in.get(f"/api/playlists/{PLAYLIST_ID}/graph?mode=artist")
