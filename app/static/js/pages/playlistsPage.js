...
export function initPlaylistsPage({ likedSongsId }) {
  ...
  function renderCard(playlist) {
    ...
    // The server decides readability; an older payload without the field is
    // treated as readable so a stale cache cannot lock the whole grid.
    if (playlist.readable !== false) {
